		...
		<n-card :bordered="false">
			<template #header>
				<div class="flex items-center">
					...
					<span class="text-lg font-medium">{{ $t('settings.ses.title') }}</span>
				</div>
			</template>
					...
					<div class="flex items-start justify-between">
						<div class="flex-1">
							...
							<div class="grid grid-cols-2 gap-8px text-12px">
								<div>
									<span class="text-desc">{{ $t('settings.ses.region') }}:</span>
									<span class="ml-4px">{{ account.region }}</span>
								</div>
								<div>
									<span class="text-desc">{{ $t('settings.ses.accessKey') }}:</span>
									<span class="ml-4px">{{ account.access_key }}</span>
								</div>
								<div v-if="account.verified_domains?.length">
									<span class="text-desc">{{ $t('settings.ses.verifiedDomains') }}:</span>
									<span class="ml-4px">{{ account.verified_domains.join(', ') }}</span>
								</div>
								<div v-if="account.send_quota">
									<span class="text-desc">{{ $t('settings.ses.quota') }}:</span>
									<span class="ml-4px">
							...
							<div v-if="account.domains?.length" class="mt-8px">
								<span class="text-desc text-12px">{{ $t('settings.ses.mappedDomains') }}:</span>
			...
			<n-empty v-else :description="$t('settings.ses.noAccounts')" />
		</n-card>

		<!-- Add/Edit Modal -->
		<n-modal v-model:show="showModal" preset="card" :title="editingAccount ? $t('settings.ses.editAccount') : $t('settings.ses.addAccount')" style="width: 600px">
			<n-form ref="formRef" :model="formData" :rules="formRules" label-placement="left" label-width="120px">
				<n-form-item :label="$t('settings.ses.name')" path="name">
					<n-input v-model:value="formData.name" :placeholder="$t('settings.ses.namePlaceholder')" />
				</n-form-item>
				<n-form-item :label="$t('settings.ses.description')" path="description">
					<n-input v-model:value="formData.description" :placeholder="$t('settings.ses.descriptionPlaceholder')" />
				</n-form-item>
				<n-form-item :label="$t('settings.ses.region')" path="region">
					<n-select v-model:value="formData.region" :options="regionOptions" :placeholder="$t('settings.ses.regionPlaceholder')" />
				</n-form-item>
				<n-form-item :label="$t('settings.ses.accessKey')" path="access_key">
					<n-input v-model:value="formData.access_key" :placeholder="$t('settings.ses.accessKeyPlaceholder')" />
				</n-form-item>
				<n-form-item :label="$t('settings.ses.secretKey')" path="secret_key">
					<n-input
						v-model:value="formData.secret_key"
						type="password"
						show-password-on="click"
						:placeholder="editingAccount ? $t('settings.ses.secretKeyPlaceholderEdit') : $t('settings.ses.secretKeyPlaceholder')" />
				</n-form-item>
				<n-form-item :label="$t('settings.ses.domains')" path="domains">
					<n-dynamic-tags v-model:value="formData.domains" />
					<p class="text-desc text-12px mt-4px">{{ $t('settings.ses.domainsHelp') }}</p>
				</n-form-item>
				<n-form-item :label="$t('settings.ses.enabled')" path="enabled">
...
const formRules: FormRules = {
	name: { required: true, message: t('settings.ses.nameRequired'), trigger: 'blur' },
	region: { required: true, message: t('settings.ses.regionRequired'), trigger: 'blur' },
	access_key: { required: true, message: t('settings.ses.accessKeyRequired'), trigger: 'blur' },
}
...
const handleDelete = (account: SESAccount) => {
	confirm({
		title: t('settings.ses.deleteConfirm'),
		content: t('settings.ses.deleteConfirmContent', { name: account.name }),
		onConfirm: async () => {
...
const handleTestConnection = async () => {
	if (!formData.region || !formData.access_key) {
		message.warning(t('settings.ses.fillCredentialsFirst'))
		return
	}
	if (!formData.secret_key) {
		message.warning(t('settings.ses.secretKeyRequired'))
		return
	}
	...
	try {
		...
		if (res && res.status === 'connected') {
			message.success(t('settings.ses.connectionSuccess'))
		}
	} catch (e) {
